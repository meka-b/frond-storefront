...
export const action = async ({ request }) => {
  ...
  const slug = formData.get('id') || title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  const subtitle = formData.get('subtitle') || '';
  const description = formData.get('description') || '';
  const badge = formData.get('badge') || '';
  const badge_class = formData.get('badge_class') || '';
  const sku = formData.get('sku') || `FR-${slug.substring(0, 3).toUpperCase()}`;
  const option_name = formData.get('option_name') || 'Pot';
  const option_style = formData.get('option_style') || 'swatch';
  const tags = formData.get('tags') || '';
  const chipsRaw = formData.get('chips') || '';
  const light_care = formData.get('light_care') || '';
  const water_care = formData.get('water_care') || '';
  const pet_care = formData.get('pet_care') || '';
  const video_url = formData.get('video_url') || null;
  const is_bestseller = formData.get('is_bestseller') === 'on' ? 1 : 0;
  const is_published = formData.get('is_published') === 'on' ? 1 : 0;
  const is_ugc = formData.get('is_ugc') === 'on' ? 1 : 0;

  const priceRaw = formData.get('price') || '';
  const compareAtPriceRaw = formData.get('compare_at_price') || '';
  const hasVariants = formData.get('has_variants') === 'on';

  // Variants and Images from JSON fields
  ...
  const chips = chipsRaw.split(',').map(c => c.trim()).filter(Boolean);
  ...
  const transaction = db.transaction(() => {
    insertProduct.run(
      slug, title, subtitle, description, badge, badge_class,
      sku, hasVariants ? option_name : 'Pot', option_style, tags, JSON.stringify(chips),
      light_care, water_care, pet_care, video_url, is_ugc, is_bestseller, is_published
    );

    if (hasVariants && variants.length > 0) {
      variants.forEach((v, i) => {
        const vid = `${slug}-${v.label.toLowerCase().replace(/[^a-z0-9]/g, '')}-${i + 1}`;
        insertVariant.run(
          vid, slug, v.label, v.hex_color || '#D8D2C4',
          Math.round(parseFloat(v.price || 0) * 100),
          v.compare_at_price ? Math.round(parseFloat(v.compare_at_price) * 100) : null,
          v.sku || `${sku}-${i + 1}`,
          parseInt(v.inventory_qty || 50),
          v.is_available ? 1 : 0,
          i + 1
        );
      });
    } else {
      // Single standard variant using the base price and compare_at_price
      const basePriceCents = priceRaw ? Math.round(parseFloat(priceRaw) * 100) : 4800;
      const baseCompareCents = compareAtPriceRaw ? Math.round(parseFloat(compareAtPriceRaw) * 100) : null;
      insertVariant.run(`${slug}-standard`, slug, 'Standard', '#D8D2C4', basePriceCents, baseCompareCents, `${sku}-01`, 50, 1, 1);
    }
    ...
    if (images.length > 0) {
      images.forEach((item, i) => {
        ...
        insertImage.run(
          `${slug}-img-${i + 1}`, slug, imgUrl, alt,
          i === 0 ? 1 : 0, i === 1 ? 1 : 0, 1, i + 1
        );

        // Also sync media_files library so admin/media doesn't report missing alt
        try {
    ...
    recos.forEach((recId, i) => {
      insertReco.run(`rec-${slug}-${recId}`, slug, recId, 'complete_look', i + 1);
    });
  });
...
export default function NewProduct() {
  ...
  const [lightCare, setLightCare] = useState('Bright, indirect light — tolerates medium light');
  const [waterCare, setWaterCare] = useState('Every 10–14 days; let the top soil dry');
  const [petCare, setPetCare] = useState('Pet friendly');
  ...
  const applyAIPayload = (payload) => {
    ...
    if (payload.general) {
      ...
      if (payload.general.slug) setSlug(payload.general.slug);
    ...
    // 3. Price & Variants Suggestion
    if (Array.isArray(payload.variantsSuggestion) && payload.variantsSuggestion.length > 0) {
      const firstV = payload.variantsSuggestion[0];
      if (firstV.price) setPrice((firstV.price / 100).toFixed(2));
      if (firstV.compareAtPrice) setCompareAtPrice((firstV.compareAtPrice / 100).toFixed(2));

      if (payload.variantsSuggestion.length > 1) {
        setHasVariants(true);
        setVariants(payload.variantsSuggestion.map(v => ({
  ...
  const addVariant = () => {
    setVariants([...variants, { label: 'Stone', hex_color: '#A9A49A', price: '62.00', compare_at_price: '', inventory_qty: 25, is_available: true }]);
  };
    ...
    <div className="space-y-6 max-w-4xl pb-16">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-[#1D2A1C] font-serif">Yeni Ürün Oluştur</h2>
            <p className="text-xs text-[#7A8377]">Kataloğa yeni bir bitki veya dekoratif ürün ekleyin.</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-[#E8E6DF] text-xs font-medium">
        {[
          ...
          { id: 'recos', label: 'Complete the Look' },
        ...
        <div className={activeTab === 'general' ? 'block' : 'hidden'}>
          <div className="bg-white p-6 rounded-xl border border-[#E8E6DF] space-y-4 shadow-2xs">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-[#5C665A] mb-1">Slug / Handle (Opsiyonel)</label>
                <input
                  type="text"
                  name="id"
                  value={slug}
                  onChange={(e) => setSlug(e.target.value)}
                  placeholder="Otomatik üretilir (örn: monstera)"
            ...
            <div>
              ...
              <input
                ...
                placeholder="Örn: The icon. Big, glossy fenestrated leaves."
                className="w-full text-xs border border-[#E0DED7] rounded-lg p-2.5 bg-[#FAF9F5] focus:bg-white focus:outline-none focus:border-[#1D2A1C]"
              />
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4 border-b border-[#E8E6DF]">
                  <div>
                    ...
                    <input
                      ...
                      name="option_name"
                      defaultValue="Pot"
                      placeholder="Örn: Pot, Size, Color"
                      className="w-full text-xs border border-[#E0DED7] rounded-lg p-2.5 bg-[#FAF9F5]"
                    />
                  </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-[#E8E6DF] space-y-6 shadow-2xs">
            <div>
              ...
              <p className="text-xs text-[#7A8377] mb-4">
                İlk görsel ana vitrin görseli, ikinci görsel fare üzerine gelindiğinde görünen hover görselidir.
              </p>
            ...
            {images.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-bold text-[#1D2A1C] uppercase font-mono">Yüklenen Galeri ({images.length})</h4>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Canlı kök garantili Monstera Deliciosa siparişi verin. Kolay bakım, şık seramik saksı seçenekleri ile kapınıza teslim."
                  className="w-full text-xs border border-[#E0DED7] rounded-lg p-2.5 bg-[#FAF9F5] focus:bg-white focus:outline-none focus:border-[#1D2A1C]"
                />
              </div>
