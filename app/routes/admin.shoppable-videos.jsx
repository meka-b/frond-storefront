...
export const loader = async () => {
  const videos = db.prepare(`
    SELECT sv.*, p.title as product_title, p.sku as product_sku
  ...
  `).all();
  return json({ videos, allProducts });
};
...
export const action = async ({ request }) => {
  ...
  if (intent === 'save_video') {
    const id = formData.get('id') || `seen-${Date.now()}`;
    ...
    const price_label = formData.get('price_label') || 'From $299.00';
    const original_price_label = formData.get('original_price_label') || null;

    db.prepare(`
      INSERT OR REPLACE INTO shoppable_videos (id, product_id, title, alt_text, video_url, poster_url, thumb_url, price_label, original_price_label, sort_order, is_active)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, (SELECT COALESCE(MAX(sort_order), 0) + 1 FROM shoppable_videos WHERE id != ?), 1)
    `).run(id, product_id, title, alt_text, video_url, poster_url, thumb_url, price_label, original_price_label, id);
...
export default function AdminShoppableVideos() {
  const { videos, allProducts } = useLoaderData();
  ...
  const [vidAltText, setVidAltText] = useState('');
  const [vidPid, setVidPid] = useState(allProducts[0]?.id || 'adansonii');
  const [vidUrl, setVidUrl] = useState('');
  const [vidPoster, setVidPoster] = useState('');
  const [vidThumb, setVidThumb] = useState('');
  const [vidPrice, setVidPrice] = useState('From $299.00');
  const [vidOrigPrice, setVidOrigPrice] = useState('');
  ...
  const startEdit = (v) => {
    if (v) {
      ...
      setVidId(v.id);
      setVidTitle(v.title);
      setVidAltText(v.alt_text || '');
      setVidPid(v.product_id);
      setVidUrl(v.video_url);
      setVidPoster(v.poster_url);
      setVidThumb(v.thumb_url);
      setVidPrice(v.price_label || 'From $299.00');
      setVidOrigPrice(v.original_price_label || '');
    } else {
      setEditingVid('new');
      setVidId('');
      setVidTitle('');
      setVidAltText('');
      setVidPid(allProducts[0]?.id || 'adansonii');
      setVidUrl('assets/video/seen-in-1.mp4');
      setVidPoster('assets/img/p-monstera-1.jpg');
      setVidThumb('assets/img/p-monstera-2.jpg');
      setVidPrice('From $299.00');
      setVidOrigPrice('');
    }
  };
  ...
  const filteredVideos = videos.filter((v) => {
    const match =
      ...
      (v.product_title && v.product_title.toLowerCase().includes(search.toLowerCase())) ||
      (v.alt_text && v.alt_text.toLowerCase().includes(search.toLowerCase()));

    if (!match) return false;
    if (filterMissingAlt) return !v.alt_text || v.alt_text.trim().length === 0;
    return true;
  });

  const missingAltCount = videos.filter((v) => !v.alt_text || v.alt_text.trim().length === 0).length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-bold text-[#1D2A1C] font-serif">"As Seen In" Alışveriş Yapılabilir Videolar</h2>
          ...
          <p className="text-xs text-[#7A8377] mt-0.5">
            Storefront'ta yer alan "As Seen In" shoppable video kartları, ürün eşleştirmeleri ve video SEO alt-text optimizasyonu.
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Video başlığı, bitki veya alt-text ara..."
      ...
      {filteredVideos.length === 0 ? (
        <div className="bg-white rounded-xl border border-[#E8E6DF] p-12 text-center text-xs text-[#7A8377]">
          Filtre kriterlerine uygun shoppable video bulunamadı.
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3.5">
          {filteredVideos.map((vid) => {
            const hasAlt = Boolean(vid.alt_text && vid.alt_text.trim().length > 0);
            return (
              <div
                key={vid.id}
              ...
              >
                ...
                <div
                  ...
                  className="aspect-[9/13] bg-[#F4F3EE] relative overflow-hidden flex items-center justify-center cursor-pointer select-none"
                >
                  <video
                    src={vid.video_url}
                    poster={vid.poster_url || vid.thumb_url}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-black/30 flex flex-col justify-between p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <AriaTooltip
                        content={
                          hasAlt
                            ? `Video SEO Alt Metni: "${vid.alt_text}"`
                    ...
                    <div>
                      ...
                      <div className="text-white/80 text-[11px] font-mono mt-0.5">
                        {vid.product_title} · {vid.price_label}
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-3 border-t border-[#E8E6DF] flex flex-col justify-between flex-1 bg-white space-y-2">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="font-mono text-[#7A8377]">Ürün ID: {vid.product_id}</span>
                    <span className="font-semibold text-[#1D2A1C]">{vid.price_label}</span>
                  </div>

                  <AriaTooltip
                    content={hasAlt ? `Alt Metni: ${vid.alt_text}` : "SEO Alt metni tanımlanmamış"}
                    position="top"
                  >
                    <div className={`text-[11px] truncate p-1.5 rounded bg-[#FAF9F5] border border-[#E8E6DF] ${hasAlt ? 'text-[#3F5E3D]' : 'text-amber-600 italic'}`}>
                      {hasAlt ? `ALT: ${vid.alt_text}` : '⚠️ ALT METNİ EKSİK'}
                    </div>
                  </AriaTooltip>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-1 border-t border-[#F0EFEB]">
                    ...
                    <fetcher.Form method="post">
                      ...
                      <button
                        type="submit"
                        onClick={(e) => {
                          if (!confirm(`"${vid.title}" videosunu silmek istiyor musunuz?`)) e.preventDefault();
                        }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl border border-[#E8E6DF] max-w-2xl w-full overflow-hidden shadow-2xl animate-in zoom-in-95 duration-150 flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8E6DF] bg-[#FAF9F5]">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-serif font-bold text-sm text-[#1D2A1C]">
                  {editingVid === 'new' ? 'Yeni Shoppable Video Ekle' : 'Shoppable Video & SEO Denetçisi'}
                </h3>
              </div>
              <button
                type="button"
                onClick={() => setEditingVid(null)}
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <fetcher.Form
                method="post"
                onSubmit={() => setEditingVid(null)}
                className="space-y-4"
              >
                <input type="hidden" name="intent" value="save_video" />
                <input type="hidden" name="id" value={vidId} />
                <input type="hidden" name="video_url" value={vidUrl} />
                <input type="hidden" name="poster_url" value={vidPoster} />
                <input type="hidden" name="thumb_url" value={vidThumb} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Örn: As Seen In — Architectural Digest"
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <AriaTooltip
                      content="Google arama motorları ve ekran okuyucular için videonun görsel içeriğini açıklayan SEO metni."
                      position="left"
                    >
                  ...
                  <input
                    ...
                    name="alt_text"
                    value={vidAltText}
                    onChange={(e) => setVidAltText(e.target.value)}
                    placeholder="Örn: Oturma odası köşe dekorasyonunda sergilenen büyük boy Fiddle-Leaf Fig bitkisi"
                    className="w-full text-xs border border-[#E0DED7] rounded-lg p-2.5 bg-[#FAF9F5] focus:bg-white focus:outline-none focus:border-[#1D2A1C]"
                  />
                  <p className="text-[11px] text-[#7A8377]">
                    Video posteri ve arama motorlarında indekslenecek erişilebilir alt metin.
                  </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-[#5C665A] mb-1">Fiyat Etiketi</label>
                    <input
                      type="text"
                      name="price_label"
                      value={vidPrice}
                      onChange={(e) => setVidPrice(e.target.value)}
                      placeholder="From $299.00 veya $199.00"
                      className="w-full text-xs border border-[#E0DED7] rounded-lg p-2.5 bg-[#FAF9F5]"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-[#5C665A] mb-1">İndirim Öncesi (Opsiyonel)</label>
                    <input
                      type="text"
                      name="original_price_label"
                      value={vidOrigPrice}
                      onChange={(e) => setVidOrigPrice(e.target.value)}
                      placeholder="$350.00"
                      className="w-full text-xs border border-[#E0DED7] rounded-lg p-2.5 bg-[#FAF9F5]"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <R2Uploader
                      ...
                      value={vidUrl}
                      onUploadComplete={setVidUrl}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#5C665A] mb-1">Kapak / Poster Görseli</label>
                    <R2Uploader label="" value={vidPoster} onUploadComplete={setVidPoster} />
                  </div>
                </div>
